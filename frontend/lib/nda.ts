...
export function getMndaTerm(data: NDAFormData): string {
  ...
  return "continued until terminated in accordance with the terms of the MNDA";
}
...
// Standard Terms sourced from CommonPaper Mutual NDA Version 1.0
...
export function getFilledSections(data: NDAFormData): StandardTermsSection[] {
  const filled = STANDARD_TERMS_TEMPLATE
    .replace(/\{\{Purpose\}\}/g, data.purpose || "[Purpose]")
    .replace(/\{\{Effective Date\}\}/g, formatDate(data.effectiveDate))
    .replace(/\{\{MNDA Term\}\}/g, getMndaTerm(data))
    .replace(/\{\{Term of Confidentiality\}\}/g, getConfidentialityTerm(data))
  ...
  return filled
    ...
    .filter((s): s is StandardTermsSection => s !== null);
}
